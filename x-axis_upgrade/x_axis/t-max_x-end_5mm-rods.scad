...
module x_end_base(vfillet=[3, 3, 3, 3], thru=true, len=40, offset=0, spring=0){
    ...
    difference(){
        ...
        translate([0, 0, 5 - bushing_xy[0]]) {  // m5 nut insert
            translate([0, 17, 0]) rotate([0, 0, 10]){
                //rod
                translate([0, -1, -2]) cylinder(h=(4.1 + nut_trap_height), r=3, $fn=32);
                //nut
                translate([0, -1, nut_trap_height-m5_nut_height]) rotate([0,0,-10]) cylinder(r1=m5_nut_diameter_horizontal/2+.1, r2=m5_nut_diameter_horizontal/2+.1+.4, h=m5_nut_height+.1, $fn=6);
    ...
    //threaded rod
    translate([0, 17-1, 0]) %cylinder(h = 70, r=2.5+0.2);
}
